...
class AIService {
  constructor() {
    if (!process.env.GEMINI_API_KEY) {
      console.warn(
        "⚠️  GEMINI_API_KEY not found. AI features will be limited."
      );
      this.genAI = null;
      return;
    }
...
Format as a professional audit summary.
  ...
  fallbackSummary(data, queryIntent) {
    if (Array.isArray(data)) {
      ...
      return `Retrieved evidence with ${keys.length} data points: ${keys.join(
        ", "
      )}. Query: "${queryIntent}"`;
    } else {
